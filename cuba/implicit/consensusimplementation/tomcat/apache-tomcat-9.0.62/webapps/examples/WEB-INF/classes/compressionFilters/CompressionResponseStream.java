...
public class CompressionResponseStream extends ServletOutputStream {
    ...
    public void write(byte b[], int off, int len) throws IOException {
        ...
        if (debug > 1) {
            // System.out.println("write, bufferCount = " + bufferCount + " len = " + len +
            // " off = " + off);
        }
        if (debug > 2) {
        ...
        if (gzipstream == null) {
            ...
            if (compressionMimeTypes != null) {
                if (startsWithStringArray(compressionMimeTypes, response.getContentType())) {
                    ...
                    if (debug > 0) {
                        // System.out.println("mime type " + response.getContentType() + " is
                        // compressible");
                    }
                } else {
                    if (debug > 0) {
                        // System.out.println("mime type " + response.getContentType() + " is not
                        // compressible");
                    }
                }
            }
            ...
            if (response.isCommitted()) {
                ...
                if (debug > 1) {
                    // System.out.print("Response already compressed. Using original output
                    // stream");
                }
                gzipstream = output;
            } else if (!compressibleMimeType) {
                if (debug > 1) {
                    // System.out.print("Response mime type is not compressible. Using original
                    // output stream");
                }
                gzipstream = output;
            } else {
