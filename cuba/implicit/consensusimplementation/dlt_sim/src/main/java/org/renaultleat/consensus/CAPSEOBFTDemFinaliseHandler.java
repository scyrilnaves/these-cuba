                    ...
                    try {
                        ...
                        if (messageType.equals("FINALISE")) {
                            ...
                            if (this.blockchain.isReadyforPartialBlockValidity(inPartialblock)) {
                                // System.out.println("fin1" + inPartialblock.getMajorBlocknumber());
                                // Check for Conflicts
                                boolean timeStampValidity = false;
                                if (!this.blockchain.partialBlockFilled(inPartialblock)) {
                                    // System.out.println("fin" + inPartialblock.getMajorBlocknumber());
                                    timeStampValidity = true;
                                } else {
                                    if (inPartialblock.getBlockTimestamp().compareTo(this.blockchain
                                            .getPartialBlockFilled(inPartialblock).getBlockTimestamp()) < 0) {
                                        timeStampValidity = true;
                                    }
                                }

                                // End of Check for Conflicts
                                if (inPartialblock.isFinalised()
                                        && this.blockchain.isValidPartialBlock(inPartialblock) && timeStampValidity) {
                                    // System.out.println("fin2" + inPartialblock.getMajorBlocknumber());
