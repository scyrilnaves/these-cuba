...
public class PartialBlock {
    ...
    // Method to update the commit details for a paricular partial block
    public void setCommitDetails(int subEpoch, int quorumId, QuorumStorage quorumStorage) {
        // Get Quorum Message for aparticular subEpoch
        QuorumMessage quorumMessage = quorumStorage.getQuorumMessageMap().get(subEpoch);
        // Map<Node Index within Quorum, Original Node Id>
        Map<Integer, Integer> quorumOriginalMapping = quorumMessage.getOriginalNodeMap().get(quorumId)
                .getQuorumOriginalMappping();
        // Get the original Wallet Node Property
        CopyOnWriteArrayList<Integer> nodeProperties = new CopyOnWriteArrayList<Integer>();
        CopyOnWriteArrayList<String> nodePropertiesString = new CopyOnWriteArrayList<String>();
        for (Map.Entry<Integer, Integer> quorumOriginalSet : quorumOriginalMapping.entrySet()) {

            nodeProperties.add(quorumOriginalSet.getValue());
            nodePropertiesString.add(String.valueOf(quorumOriginalSet.getValue()));
        }

        this.setCommitMessageValidators(nodePropertiesString);
        this.setCommitMessageValidatorIndexes(nodeProperties);
